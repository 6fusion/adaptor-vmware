AdaptorVMware.controllers :machines, :parent => :inodes do
  ...
  post :index do
    ...
    @machine = Machine.create(@inode, options["account_id"], options["media_store_location"], options["ovf_file_path"], options["virtual_machine_uuid"], options["network_maps"], options["disk_maps"])
