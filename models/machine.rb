        ...
        stats.sampleInfo.each_with_index.map do |x, i|
          if stats.value.empty?.eql?(false)
            ...
            result << MachineReading.new({
                                           ...
                                           :cpu_usage    => metric_readings[cpu_metric_usage].nil? ? 0 : metric_readings[cpu_metric_usage][i] == -1 ? 0 : (metric_readings[cpu_metric_usage][i].to_f / 100**100).to_f,
            ...
            logger.info("CPU Metric Usage="+((metric_readings[cpu_metric_usage][i].to_f / 100**100).to_s))
